...
resource "proxmox_vm_qemu" "k8s_master_secondary" {
  name        = var.vm_hostname_master_aux
  desc        = "Kubernetes secondary master node for HA"
  target_node = var.proxmox_node
  vmid        = var.vm_id_secondary  # Add this variable

  # ... (same configuration as k8s_master, but different name/vmid)
  clone = var.vm_template_name
  full_clone = true
  ...
  tags = "k8s;linux;critical;master;auxiliary"
